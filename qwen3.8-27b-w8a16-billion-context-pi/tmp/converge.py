...
def trace(b, dt, maxsteps=2000000, rmax=300.0):
    p=np.array([-150.0,b,0.0]); v=np.array([1.0,0.0,0.0])
    ...
    ang=np.degrees(np.arccos(np.clip(np.dot([1,0,0.0],vhat),-1,1)))
    return ang, minr, False
print("b=10:  dt        deflection   (exact 31.53)")
for dt in [0.002, 0.001, 0.0005, 0.0002]:
    a,mr,cap = trace(10.0, dt)
    print(f"   {dt:8}  {a:10.4f}  minr={mr:.4f} cap={cap}")
